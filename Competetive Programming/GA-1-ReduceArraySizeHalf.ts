...
class MaxHeap {
    ...
    add(el){
        ...
        while(i>0 && this._q[i].val>this._q[~~((i-1)/2)].val) {
            [this._q[i], this._q[~~((i-1)/2)]] = [this._q[~~((i-1)/2)], this._q[i]];
            i = ~~((i-1)/2);
        }
        this.root = this._q[0];
    }
    get(){
        ...
        [this._q[0], this._q[this._q.length-1]] = [this._q[this._q.length-1], this._q[0]];
        this._q.pop();
        this._heapify(0);
    ...
    _heapify(index){
        ...
        if(2*index+1<this._q.length && this._q[2*index+1].val > this._q[max].val) {
            max = 2*index+1;
        }
        
        if(2*index+2<this._q.length && this._q[2*index+2].val >  this._q[max].val) {
            max = 2*index+2
        }
